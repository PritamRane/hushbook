...
mongoose.connect(
  "mongodb+srv://root:root@cluster0.qlhxjdj.mongodb.net/?retryWrites=true&w=majority",
  {
...
app.post("/api/addingbooks", (req, res, next) => {
  ...
  UserTest.findOneAndUpdate(
    { _id: user1._id },
    { $set: { books: user1.books } },
    { new: true },
    (err, doc) => {
      if (err) {
...
app.post("/api/addingFavbooks", (req, res, next) => {
  ...
  UserTest.findOneAndUpdate(
    { _id: user1._id },
    { $set: { favorites_list: user1.favorites_list } },
    { new: true },
    (err, doc) => {
      if (err) {
        console.log("error happened in adding book");
